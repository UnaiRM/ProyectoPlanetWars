...
as
   ...
   procedure RETURN_PLANET_SHIP(id_p int, res out varchar);
   
   procedure RETURN_PLANET_DEFENSE(id_p int, res out varchar);
...
------------------------------------------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------------------------------------------------
    ...
    exception
        ...
        when others then 
        dbms_output.put_line('ERROR: NO SE HA INDENTIFICADO EL ERROR INSERT_USER');
    
    end;
-----------------------------------------------------------------------------------------------------------------------------
   procedure RETURN_PLANET_SHIP(id_p int, res out varchar)
   as
      cursor c1 is select id_ship, quantity, level_defense, level_atack from planet_ship where id_planet = id_p;
      all_c varchar(500);
      id_d int;
      quant int;
      lvl_d int;
